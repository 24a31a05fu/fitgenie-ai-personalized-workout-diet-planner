...
const Router = {
    ...
    navigate(pageId) {
        ...
        // Set active in drawer
        const drawerActiveLinks = document.querySelectorAll('.drawer-link');
        drawerActiveLinks.forEach(link => link.classList.remove('active'));
        const activeDrawerLink = document.querySelector(`.drawer-link[data-target="${pageId}"]`);
        if (activeDrawerLink) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Setup click handlers for drawer links
    const drawerLinks = document.querySelectorAll('.drawer-link');
    drawerLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            const targetPage = link.getAttribute('data-target');
            
            if (link.id === 'drawer-nav-features-guest') {
                closeDrawer();
                const landing = document.getElementById('landing-page');
                const features = document.getElementById('features-header');
                if (landing && features) {
                    landing.scrollIntoView({behavior:'smooth'});
                    setTimeout(() => features.scrollIntoView({behavior:'smooth'}), 150);
                }
            } else if (link.id === 'drawer-nav-about-guest') {
                closeDrawer();
                const landing = document.getElementById('landing-page');
                const testimonials = document.getElementById('testimonials-header');
                if (landing && testimonials) {
                    landing.scrollIntoView({behavior:'smooth'});
                    setTimeout(() => testimonials.scrollIntoView({behavior:'smooth'}), 150);
                }
            } else if (link.id === 'drawer-nav-login-guest') {
                closeDrawer();
                if (window.AuthModal) window.AuthModal.showLogin();
            } else if (link.id === 'drawer-nav-register-guest') {
                closeDrawer();
                if (window.AuthModal) window.AuthModal.showSignup();
            } else if (link.id === 'drawer-nav-logout-btn') {
                closeDrawer();
